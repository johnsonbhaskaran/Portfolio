import { useTheme } from "../themeContext";
import { Check } from "feather-icons-react";
import LoadingLight from "./../assets/Loading_light.svg";
import LoadingDark from "./../assets/Loading_dark.svg";

const Prowess = () => {
  const { theme } = useTheme();

  return (
    <div className='flex justify-evenly w-full h-full'>
      <div className='flex flex-col justify-evenly items-center xl:items-start md:min-w-5xl max-w-xs h-screen'>
        <h2 className='text-[32px] md:text-5xl xl:text-6xl xl:text-left text-center'>
          My Prowess 💪
        </h2>
        <div
          className={`flex flex-wrap xl:justify-start justify-center gap-4 max-w-2xl xl:max-w-4xl ${
            theme === "light" ? "text-white" : "text-[#333]"
          }`}
        >
          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <Check
              className={`${
                theme === "light" ? "bg-white text-[#6A5ACD]" : "bg-[#333] text-[#9ACD32]"
              } p-0.5 rounded-full size-5 md:size-8 `}
            />
            JavaScript
          </div>
          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <Check
              className={`${
                theme === "light" ? "bg-white text-[#6A5ACD]" : "bg-[#333] text-[#9ACD32]"
              } p-0.5 rounded-full size-5 md:size-8 `}
            />
            React
          </div>
          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <Check
              className={`${
                theme === "light" ? "bg-white text-[#6A5ACD]" : "bg-[#333] text-[#9ACD32]"
              } p-0.5 rounded-full size-5 md:size-8 `}
            />
            Node
          </div>
          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <Check
              className={`${
                theme === "light" ? "bg-white text-[#6A5ACD]" : "bg-[#333] text-[#9ACD32]"
              } p-0.5 rounded-full size-5 md:size-8 `}
            />
            HTML
          </div>
          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <Check
              className={`${
                theme === "light" ? "bg-white text-[#6A5ACD]" : "bg-[#333] text-[#9ACD32]"
              } p-0.5 rounded-full size-5 md:size-8 `}
            />
            Express
          </div>
          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <Check
              className={`${
                theme === "light" ? "bg-white text-[#6A5ACD]" : "bg-[#333] text-[#9ACD32]"
              } p-0.5 rounded-full size-5 md:size-8 `}
            />
            MongoDB
          </div>

          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <Check
              className={`${
                theme === "light" ? "bg-white text-[#6A5ACD]" : "bg-[#333] text-[#9ACD32]"
              } p-0.5 rounded-full size-5 md:size-8 `}
            />
            CSS
          </div>
          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <Check
              className={`${
                theme === "light" ? "bg-white text-[#6A5ACD]" : "bg-[#333] text-[#9ACD32]"
              } p-0.5 rounded-full size-5 md:size-8 `}
            />
            Canva
          </div>
          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <Check
              className={`${
                theme === "light" ? "bg-white text-[#6A5ACD]" : "bg-[#333] text-[#9ACD32]"
              } p-0.5 rounded-full size-5 md:size-8 `}
            />
            Figma
          </div>
          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <Check
              className={`${
                theme === "light" ? "bg-white text-[#6A5ACD]" : "bg-[#333] text-[#9ACD32]"
              } p-0.5 rounded-full size-5 md:size-8 `}
            />
            Photoshop
          </div>
          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <Check
              className={`${
                theme === "light" ? "bg-white text-[#6A5ACD]" : "bg-[#333] text-[#9ACD32]"
              } p-0.5 rounded-full size-5 md:size-8 `}
            />
            Illustrator
          </div>
          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <img
              className='scale-65 md:scale-100'
              src={theme === "light" ? LoadingDark : LoadingLight}
              alt=''
            />
            Python
          </div>
          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <img
              className='scale-65 md:scale-100'
              src={theme === "light" ? LoadingDark : LoadingLight}
              alt=''
            />
            Java
          </div>
          <div
            className={`${
              theme === "light" ? "bg-[#6A5ACD]" : "bg-[#9ACD32]"
            } px-4 py-2 md:px-8 md:py-4 cursor-pointer rounded-full md:text-3xl flex items-center gap-2`}
          >
            <img
              className='scale-65 md:scale-100'
              src={theme === "light" ? LoadingDark : LoadingLight}
              alt=''
            />
            Spline
          </div>
        </div>
      </div>
    </div>
  );
};
export default Prowess;
